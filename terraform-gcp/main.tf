...
resource "google_service_account" "cpu-node_sa" {
  account_id   = "cpu-node-sa"
  display_name = "CPU Node Service Account"
}

resource "google_project_iam_member" "cpu-node_log_writer" {
  project = var.project_id
  role    = "roles/logging.logWriter"
  member  = "serviceAccount:${google_service_account.cpu-node_sa.email}"
}

resource "google_project_iam_member" "cpu-node_metric_writer" {
  project = var.project_id
...
resource "google_compute_instance" "cpu-node" {
  name         = "ai-cpu-node"
  ...
  boot_disk {
    initialize_params {
      # Standard Debian 11 image for CPU usage
  ...
  service_account {
    email  = google_service_account.cpu-node_sa.email
...
resource "google_compute_instance_group" "cpu_group" {
  name      = "ai-cpu-group"
  zone      = var.zone
  instances = [google_compute_instance.cpu-node.self_link]
